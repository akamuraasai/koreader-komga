...
describe("Selection.chosen", function()
  it("returns selected books in input order", function()
    local sel = { ["2.5"] = true, ["4"] = true }
    local chosen = S.chosen(books, function(id) return sel[id] end)
    assert.same({ "2.5", "4" }, { chosen[1].id, chosen[2].id })
  end)
  it("returns empty when nothing is selected", function()
    assert.same({}, S.chosen(books, function() return false end))
  end)
end)

describe("Selection.allIds", function()
  it("returns every book id in input order", function()
    assert.same({ "1", "2", "2.5", "3", "4" }, S.allIds(books))
  end)
end)
